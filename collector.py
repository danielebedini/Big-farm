...
def cl_req1(conn,addr): # TODO: test this
  ...
  for sum,fn in c_list:
    conn.sendall(struck.pack("!i", sum))
    conn.sendall(fn.encode())
  
def cl_req2(conn,addr,new_sum): # TODO: test this
  ...
  # send length first then send couples
    conn.sendall(struck.pack("!i", len(new_l)))
    for sum,fn in new_l:
      conn.sendall(struck.pack("!i", sum))
      conn.sendall(fn.encode())
  else:
    # if the new list is empty
    conn.sendall("Nessun file".encode())
